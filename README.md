...
│   ├── commands.yaml                   ← Maps bin/dev commands to real build/test scripts
...
| Starting from… | Read… |
|----------------|-------|
| **A blank slate** (new project, no code yet) | Continue below — Steps 1–7 |
| **An existing repo** (has code, tests, build scripts) | **[ADOPTION.md](./ADOPTION.md)** — phased guide for dropping SDD into an existing project, including `bin/dev` setup |

> **`bin/dev` deep-dive:** For a full explanation of how the CLI wrapper works, its architecture, smart verification, git flow commands, and how to extend it — see [ADOPTION.md § How `bin/dev` Works](./ADOPTION.md#how-bindev-works).

---

Follow these steps to adopt SDD for a brand-new project. The whole process takes about 10 minutes.
...
A: Modify the plan directly before approving it. You can rewrite the specification, change stage instructions, adjust the manifest, or add constraints. Once you set `approval.status: approved` in `manifest.yaml`, the plan is the final spec — the executing agent follows it exactly.
...
A: The `manifest.yaml` tracks `current_stage` and each stage's `status`. When you start a new agent conversation to resume, the agent reads the manifest, sees which stages are already `done`, and picks up from where it left off. See `agent-development/agent-specs/agent-workflow.md` for the full resumption protocol.
